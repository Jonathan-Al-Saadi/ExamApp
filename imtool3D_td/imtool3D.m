...
classdef imtool3D < handle
    ...
    methods
        ...
        function tool = imtool3D(varargin)  %Constructor
            ...
            % Add a new button for Time-ROI
            tool.handles.Tools.TimeROI = uicontrol(tool.handles.Panels.ROItools, 'Style', 'pushbutton', 'String', '', 'Position', [buff buff+w*4 w w], 'TooltipString', 'Create Time ROI');
            icon_time = makeToolbarIconFromPNG([MATLABicondir '//tool_shape_ellipse.png']);
            set(tool.handles.Tools.TimeROI, 'Cdata', icon_time);
            fun_time = @(hObject, evnt) measureTimeROICallback(hObject, evnt, tool);
            set(tool.handles.Tools.TimeROI, 'Callback', fun_time);
            
            %Create Square ROI button
            tool.handles.Tools.SquareROI           =   uicontrol(tool.handles.Panels.ROItools,'Style','pushbutton','String','','Position',[buff buff+2*w w w],'TooltipString','Create Time ROI');
...
switch type
    ...
    case 'rectangle'
        ...
        % Store the ROI in a global variable
        tool.roi = ROI;

        % Create and update the ROI figure
        createAndUpdateROIFigure(tool);
...
function createAndUpdateROIFigure(tool)
% Create a figure to display the ROI values
tool.handles.hFig = figure('Name', 'Time - Intensity curve', 'NumberTitle', 'off', 'Position', [100, 100, 300, 150]);
hAx = axes('Parent', tool.handles.hFig);
xlabel(hAx, 'Time');
ylabel(hAx, 'Absolute instenisty');
title(hAx, 'Time - Intensity curve');

% Update the figure with the initial ROI values
updateROIFigure(tool.roi, hAx, tool);

% Add a callback to update the figure when the ROI is moved
addlistener(tool.roi, 'newROIPosition', @(src, evt) updateROIFigure(src, hAx, tool));
end

function updateROIFigure(roi, hAx, tool)
% Get the current position of the ROI
pos = roi.getPosition();

% Extract x, y, and z values (if applicable)
x = pos(:, 1);
y = pos(:, 2);
z = tool.getCurrentSlice;  % Assuming z-values are available in the third column

%Get vol
volCell = tool.getImage(1);
vol = volCell{1,1}; 

% Check if vol is 4D
if ndims(vol) ~= 4
    % Display error message on the plot
    text(hAx, 0.5, 0.5, 'Image is not 4D', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'FontSize', 12, 'Color', 'red');
    title(hAx, 'Invalid Image Dimension');
    return;
end

%Get Mask
roiMeasurements = roi.getMeasurements;
maskROI = roiMeasurements.mask;

% Ensure maskROI is applied to the correct slice
mask4D = false(size(vol)); % Create a 4D mask
mask4D(:, :, z, :) = repmat(maskROI, [1, 1, 1, size(vol, 4)]); % Place the 2D mask in the correct slice and replicate it across the time dimension

% Apply the 4D mask to the 4D volume
maskedVol = vol .* int16(mask4D);

% Extract the non-zero values within the mask across time
roiValuesOverTime = maskedVol(mask4D);

% Reshape to separate time dimension
roiValuesOverTime = reshape(roiValuesOverTime, [], size(vol, 4));

% Calculate mean signal intensities over time for the ROI
signalIntensities = mean(roiValuesOverTime, 1);

% Calculate standard deviation of signal intensities over time for the ROI
stdSignalIntensities = std(double(roiValuesOverTime), 0, 1);

% Plot the signal intensities over time with error bars
errorbar(hAx, 1:size(vol, 4), signalIntensities, stdSignalIntensities);
